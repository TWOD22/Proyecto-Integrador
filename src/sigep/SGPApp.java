...
public class SGPApp {
    ...
    public static void main(String[] args) {
        // Manejador global para errores no controlados: registra trazas y muestra diálogo.
        Thread.setDefaultUncaughtExceptionHandler((thread, ex) -> {
            try {
                ex.printStackTrace();
                java.io.File f = new java.io.File("error_log.txt");
                try (java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.FileWriter(f, true))) {
                    pw.println("---- Uncaught exception: " + java.time.Instant.now() + " ----");
                    ex.printStackTrace(pw);
                }
            } catch (Exception ignore) {}
            try { SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(null, "Ocurrió un error inesperado:\n" + ex, "Error", JOptionPane.ERROR_MESSAGE)); } catch (Exception ignore) {}
        });
